...
**Steps and approach:** The architecture already has foundational pieces in place — the **image-rasterstore** S3 bucket is federated with ArcGIS Image Server via Portal, the PostgreSQL master databases (PROD and UAT) host mosaic datasets that can back image services, and imagery is archived in COG format on I: eo_image. The strategy builds outward from these components rather than introducing parallel infrastructure.
